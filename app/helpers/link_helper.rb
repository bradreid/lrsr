module LinkHelper
  ...
  def action_links(context, opts = {})
    ...
    index_path = index_offset == -1 ? context_index(context) : context[0..index_offset] unless options[:exclude].include?(:index)
    a = []
    a << [link_to(options[:index][:title], opts[:index_path] || polymorphic_path(index_path),  {:class => 'back_icon'}.merge(options.merge(options[:index] || {})))] unless options[:exclude].include?(:index)
    a << [ view_link_if_permitted(context,  {:class => 'view_icon'}.merge(options.merge(options[:show] || {})))] unless options[:exclude].include?(:show)
    a << [ edit_link_if_permitted(context,  {:class => 'edit_icon'}.merge(options.merge(options[:edit] || {})))] unless options[:exclude].include?(:edit)
    a << [ delete_link_if_permitted(context,  {:class => 'delete_icon'}.merge(options.merge(options[:delete] || {})))] unless options[:exclude].include?(:delete)
    a.flatten.compact.join(' ').html_safe
  end
